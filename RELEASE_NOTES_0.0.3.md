...
- **Tests**: Verilator sim builds; cocotb smoke (icarus) for IRQ_TEST/DMA done; RTL DMA loopback SV test in CI; HDMI CRC SV logger added (no golden yet). Optional CMake host build runs in CI.
...
## Known limitations
- HDMI CRC golden value not set; stubbed pipeline doesn’t emit frames in the minimal SV bench yet—CRC test logs frame/CRC only.
