...
bool SDLBackend::init(PlatformContext& ctx, const PlatformConfig& cfg) {
    ...
    if (cfg.vsync) renderer_flags |= SDL_RENDERER_PRESENTVSYNC;
    ...
    context = new SdlContext{window, renderer, texture, cfg.width, cfg.height};
    ctx.user = context;
    return true;
}
...
void SDLBackend::present(PlatformContext& ctx, const uint32_t* pixels, int w, int h) {
    ...
    if (!ensure_texture(*context, w, h)) return;

    void* tex_pixels = nullptr;
    int pitch_bytes = 0;
    if (SDL_LockTexture(context->texture, nullptr, &tex_pixels, &pitch_bytes) == 0) {
...
void SDLBackend::shutdown(PlatformContext& ctx) {
    if (context) {
