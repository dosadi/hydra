...
- Add an `ai-dashboard` CI job that runs `scripts/ai_health_dashboard.py`, uploads `out/ai_health_dashboard.txt`, and surfaces unknown-priority clusters for reviewers before merging, then posts the summary directly into the workflow run summary for easy visibility. Run `scripts/ai_dashboard_briefing.py` afterwards so `docs/todo/todo_ai_dashboard.md` stays synchronized.
