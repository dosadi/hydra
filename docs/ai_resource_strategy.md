...
- Add an `ai-dashboard` CI job that runs `scripts/ai_health_dashboard.py`, uploads `out/ai_health_dashboard.txt`, and surfaces unknown-priority clusters for reviewers before merging, then posts the summary directly into the workflow run summary for easy visibility. Record resulting actions in `docs/todo/todo_ai_dashboard.md` so the dashboard tracker stays aligned with automation.
