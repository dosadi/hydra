...
- **TODO [P2]:** Align the build toolchain by touching `sim/Makefile` + Verilator integration with a top-level CMake preset and cross-link the new macros in `README.md`/`docs/macos_windows_build.md`; this keeps tree navigation consistent between simulators, SDK tools, and platform jobs.
- **DONE [P1]:** Implement pervasive doc touch system (Makefile-style dependency tracking) via `scripts/doc_touch.py` to flag stale documentation when dependencies change; see `docs/doc_touch_system.md` for complete guide.
