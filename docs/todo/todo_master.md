# Hydra Task Tracker (toward 0.0.7)
...
## Simulation / Viewer
- IN-PROGRESS [P2]: Diagnose 'o' key (diagnostic slice toggle) not responding - added debug output (commit 708a373), awaiting user test results.
...
- TODO [P2]: Add a small “record inputs to script” mode (log keys/mouse deltas with timestamps) and a “playback” mode for deterministic repros.
- DONE: Emit a brief startup summary (backend, font path, env knobs in effect) to stderr to aid reproducibility in logs.
- TODO [P2]: Offer a HUD toggle for memory/bandwidth counters exposed from RTL (mem_cycle/read/write) so perf data is visible without LOG_FRAMES spam.
...
- TODO [P2]: Add a grid/axis overlay toggle to help orientation inside the voxel volume.
- TODO [P2]: Add joystick/gamepad input support (fallback to SDL game controller mappings).
...
- TODO [P2]: Add crosshair style/color/size customization (env/hotkey) so recorded clips can match different backgrounds.
- TODO [P2]: Add an option to log HUD stats (FPS, mem util, hits) to a CSV for offline perf tracking.
...
- IN-PROGRESS [P2]: Add a tiny config file parser (e.g., env-var-provided path) to preload camera/flags/backend/seed (autosave/autoload exists; extend to seed/backend).
...
- TODO [P2]: Add a latency/profiling overlay (ms/frame breakdown) toggled by a hotkey for perf debugging.
- TODO [P2]: Add a toggle to freeze camera but keep rendering (to inspect static scenes) and another to pause rendering ticks.
...
- TODO [P2]: Add a “single-step frame” hotkey (advance one frame) for debugging frame_done and HUD rendering.
...
- TODO [P2]: Add a “reset world” hotkey/env to re-run the procedural generator and reload into BRAM without restarting sim.
- TODO [P2]: Add a toggle to visualize selection bounding box/normal in 3D (e.g., wireframe highlight).
- TODO [P2]: Add a debug mode that shows cam vectors (dir/plane) as on-screen arrows for math sanity checks.
- DONE: Add keybind to dump current camera/flag/selection state to stdout for copy-paste into tests/docs (P key).
- TODO [P2]: Add optional gamma correction or tone-mapping toggle to make visuals more consistent across displays.
...
- TODO [P2]: Add an env/hotkey to choose mouse smoothing vs. raw input (helpful for touchpads).
...
- TODO [P2]: Add a hotkey/env to toggle cursor ray visualization (line to hit point) for debugging aiming/selection.
- TODO [P2]: Add an audio click/feedback on selection or edit to improve UX during demos (optional, env-controlled).
- TODO [P2]: Add a seed/randomize hotkey to quickly flip worlds without restarting the sim.
- TODO [P2]: Add a “demo mode” that runs a scripted camera path and toggles flags for capture reels.
- TODO [P2]: Add per-axis inversion/sensitivity sliders (env/CLI) for finer camera tuning.
- DONE: Add a HUD color theme toggle (light/dark) to improve readability on different backgrounds (T hotkey for theme toggle).
- TODO [P2]: Add a "camera jitter" noise option to stress anti-aliasing/perf stability.
- TODO [P2]: Add a HUD overlay to show current seed and config file path when loaded.
...
- TODO [P2]: Add a “cursor highlight” toggle to show the selected voxel with a bright outline for clarity in recordings.
- TODO [P2]: Add a “telemetry off” mode to skip HUD drawing for maximal render throughput in benchmarks.
...
- TODO [P2]: Tie off or assert stub AXI master signals in `rtl/voxel_axi_core.sv` to silence unused-interface warnings (aw/ar/w channels driven with valid=0 today).
- TODO [P2]: Surface or assert the `pixel_reemissure` sideband in `rtl/voxel_axi_core.sv` so the 96-bit format stays exercised.
...
- TODO [P2]: Keep AXI-Lite coverage tracked in `docs/todo/todo_axi_lite_coverage.md` and add regression scripts/tests as coverage requirements evolve.
- TODO [P2]: Add compile-time parameters or CSRs for VOXEL_GRID_SIZE/SCREEN dims that propagate into the sim HUD for consistency.
- TODO [P2]: Add an assertion or coverage point for `frame_done` cadence vs. expected pixel count to catch truncated frames in RTL.
...
- TODO [P2]: Add coverage/assertions that INT_MASK gates irq_out/msi_pulse correctly on each bit.
- TODO [P2]: Add a lint-time check or assertion that camera/flag pulses are single-cycle to avoid missed updates.
- TODO [P2]: Add a lightweight formal/cover for dbg write path (address range, pulse duration) to catch off-by-one errors.
- TODO [P2]: Add parameterized address widths for dbg write addr to match different VOXEL_GRID_SIZE targets cleanly.
- TODO [P2]: Add optional perf counters for pixel drops/STALLs in the AXI-Stream path to surface backpressure issues.
- TODO [P2]: Add a compile-time switch to bypass auto-start frames (AUTO_START_FRAMES=0) and verify manual start_frame path.
...
- TODO [P2]: Add a “sim slow” knob to inject wait-states into mem-side counters to stress stalls/backpressure paths.
- TODO [P2]: Add reset/soft-reset behavior checks to ensure counters and FSMs go to known state.
- TODO [P2]: Add coverage for diag_slice flag effects (ensuring expected pixel_write_en pattern changes when set).
- TODO [P2]: Add a small SV covergroup on INT_STATUS bits to ensure each event is observed at least once in regression.
- TODO [P2]: Add a parameter to disable diagnostic slice entirely for FPGA builds (synth-friendly switch).
- TODO [P2]: Add a build-time flag to reduce MAX_RAY_STEPS for faster sims and verify behavior under short rays.
...
- TODO [P2]: Add parameterized screen dims for non-480x360 builds and propagate to TOTAL_PIXELS checks.
- TODO [P2]: Add a cover/check that dbg writes do not coincide with soft_reset/start_frame (or define behavior).
- TODO [P2]: Add an assertion that frame_done clears core_busy within a bounded number of cycles.
- DONE: Add a check that no pixel writes occur after frame_done until the next frame start (voxel_axi_core SVA).
- TODO [P2]: Add a parameter to disable extra_light flag effects for baseline testing.
- TODO [P2]: Add coverage on selection edit paths (C/X/Z/B) to ensure dbg write pulses reach voxel RAM in sim.
- TODO [P2]: Add a simple check that fb_base/fb_stride CSRs hold non-zero values when start_frame pulses (or define default).
- TODO [P2]: Add an assertion that diag_slice changes only take effect on flags_load to avoid glitches mid-frame.
- TODO [P2]: Add a lightweight SV monitor that logs first/last pixel addresses per frame for debugging.
- TODO [P2]: Add coverpoints for MSI pulse generation on frame_done/dma_done/blit_done bits independently.
- TODO [P2]: Add an assertion that dbg_write_en is single-cycle and deasserted on the following cycle.
- TODO [P2]: Add an option to gate INT_STATUS/frame_done by start_frame so accidental asserts during reset are ignored.
- TODO [P2]: Add a cover/metric on pixels_this_frame vs. TOTAL_PIXELS to flag partial frames in sim logs.
- TODO [P2]: Add a lint/fail if VOXEL_GRID_SIZE is not a power of two (or document constraints).
- TODO [P2]: Add a coverage check that diag_slice renders touch all quadrants over a few frames.
- TODO [P2]: Add a formal/cover on FSM states for the raycaster pipeline to catch illegal transitions.
- TODO [P2]: Add a simple assertion that fb_stride is a multiple of screen width (or document if not required).
- TODO [P2]: Add an option to zero pixel outputs when rst_n is asserted mid-frame to avoid partial garbage.
...
- TODO [P2]: Add a debugfs knob in `hydra_pcie_drv` to toggle verbose IRQ logging without recompiling.
...
- TODO [P2]: Add a self-test debugfs entry to trigger IRQ_TEST and report INT_STATUS to validate interrupts without user tooling.
- TODO [P2]: Teach `hydra_dma_blit_demo` to skip cleanly (exit 77) when the device node is missing, for nicer CI gating.
- DONE: Add a tiny `hydra_cam_reset` CLI that sets camera/flags/selection to defaults via libhydra (mirrors sim reset hotkey).
- TODO [P2]: Add a simple DRM stub “modeset info” tool to enumerate any registered render node and dump its Hydra-specific props.
- TODO [P2]: Add a kernel tracepoint or trace_printk path for IRQ/DMA events to aid debugging without extra printk noise.
...
- TODO [P2]: Add a kselftest-style script in `scripts/` to exercise ioctl/mmap paths and report pass/fail.
...
- TODO [P2]: Add a debugfs entry that dumps recent IRQ timestamps/counts for profiling interrupt cadence.
- DONE: Add a simple mmap smoke test in libhydra (map BAR0/BAR1, read ID regs) for bring-up scripts.
- TODO [P2]: Add a “no-op” ioctl in kernel driver for compatibility/version probing (returns driver version/build).
- TODO [P2]: Add a CLI wrapper to run the kselftest suite and summarize results (pass/fail counts).
- TODO [P2]: Add a `hydra_dump_csrs` tool that dumps a CSR range to help compare against spec during bring-up.
- DONE: Add per-arch cross-compile notes (e.g., aarch64 cross) for the driver and libhydra.
- TODO [P2]: Add basic manpages or `--help` output for the userland tools (blit_smoketest, dma_blit_demo, drm_info).
- TODO [P2]: Add a basic perf test that times repeated HYDRA_IOCTL_RD32/WR32 calls to gauge BAR latency.
- TODO [P2]: Add a small tool that writes/reads selection/camera via the driver to mirror sim controls (for HW parity).
- TODO [P2]: Add an optional sysfs entry to expose BAR sizes/IDs (read-only) for quick inspection without debugfs.
- TODO [P2]: Add DKMS packaging script or notes for the Linux driver for easier installs.
...
- TODO [P2]: Add a module param to disable debugfs creation for locked-down environments.
- TODO [P2]: Add a tiny tool to map BAR1 (when present) and hexdump a small range for sanity.
- TODO [P2]: Add a DRM stub ioctl negative test to ensure proper error codes on bad args.
- TODO [P2]: Add a helper script to load/unload the driver with module params (MSI, IDs) for quick iteration.
- TODO [P2]: Add a CI-friendly script to run all userland tools with `--help` to ensure they parse options.
- DONE: Add a libhydra version getter and expose it in tools for troubleshooting.
- TODO [P2]: Add a kselftest case that exercises unaligned IOCTL offsets to ensure -EINVAL is returned.
- TODO [P2]: Add a libhydra call to fetch BAR sizes/info (mirrors HYDRA_IOCTL_INFO) for convenience.
- TODO [P2]: Add CI smoke that runs libhydra samples under strace to confirm IOCTL sequences look sane.
...
- TODO [P2]: Add PIC + install/export rules for libhydra in CMake for downstream consumers.
...
- TODO [P2]: Teach CI to capture and publish `sim/build/frame_diff.log` and HUD screenshots on test failures for quicker triage.
...
- TODO [P2]: Add formatting checks (clang-format for C/C++, verible/svformat for SV) to keep diffs clean.
...
- TODO [P2]: Add a CI job that runs `make test_frame` with HYDRA_BACKEND=SDL and HYDRA_BACKEND=GL (when available) to catch backend regressions.
- TODO [P2]: Add a nightly CI job to run cocotb smoke (`sim/tests/cocotb_hydra`) when tools are present, but mark non-blocking.
- TODO [P2]: Add a CI artifact upload for failing frame dumps (PPM) to speed visual diffing.
...
- TODO [P2]: Add a `make coverage` (if feasible) to gather line coverage from C++ sim tests, documented as experimental.
- DONE: Add a `make bench` target for any performance microbenchmarks or frame timing scripts.
- TODO [P2]: Add a CI matrix that runs `make test_frame` with GL/Vulkan off/on (when supported) behind a feature flag.
- DONE: Add a quick “lint-docs-links” script to fail if README references missing files.
- TODO [P2]: Add a CI job to build the Linux driver with `W=1` (sparse/extra warnings) to catch kernel API drift early.
...
- TODO [P2]: Add a container/devcontainer or Dockerfile for a known-good toolchain (Verilator, SDL2, etc.).
- DONE: Add a `make package` target to bundle sim binaries/tests/docs into an artifact tarball.
- TODO [P2]: Add a minimal “host-only” CI job that just builds CMake preset without RTL to guard host tools.
...
- TODO [P2]: Add a CI check that running `make clean` leaves the tree tidy (no staged changes).
- TODO [P2]: Add CI to run Python linters (ruff/black-check) on scripts/ to catch style issues early.
...
- TODO [P2]: Add a CI job that builds/runs `sim/tests/run_rtl_tests.sh` when iverilog/vvp are present, marking non-fatal otherwise.
- DONE: Add a `make shellcheck` target to lint bash scripts (hydra_dev_loop.sh, fetch_ip.sh, etc.).
- TODO [P2]: Add a helper script to bump version numbers across README/CMake/RELEASE_NOTES consistently.
- TODO [P2]: Add a CI job to run `scripts/hydra_dev_loop.sh` in best-effort mode to mirror developer flow.
- TODO [P2]: Add a quick gate to warn when generated files are manually edited (if detectable).
...
- TODO [P2]: Refresh IDs/rev/build in `docs/hydra_spec.md` to the current (0.0.5/next) values.
...
- TODO [P2]: Add a brief “architecture at a glance” diagram or ASCII map linking RTL modules to sim components for onboarding.
...
- TODO [P2]: Add a short “how to repro a frame” doc snippet (FRAME_DUMP/AUTO_EXIT usage with expected outputs).
- TODO [P2]: Document cocotb smoke expectations and how to interpret failures/timeouts for `sim/tests/cocotb_hydra`.
- TODO [P2]: Add a contributor note explaining status tags (TODO/IN-PROGRESS/DONE/WONTFIX-0.0.6) and how to update owners.
- TODO [P2]: Add a release checklist for 0.0.6 (tests to run, docs to touch, version bumps).
...
- TODO [P2]: Add a short primer on interpreting HUD perf counters and mem utilization readings.
...
- TODO [P2]: Add a brief note on expected memory footprint/frame times in sim for baseline hardware.
- TODO [P2]: Add a small “perf tuning” doc for sim (env vars, pacing, disable HUD) for capturing reproducible metrics.
- TODO [P2]: Add instructions for running cocotb tests with alternative simulators (icarus/vcs/etc.) if applicable.
...
- TODO [P2]: Add a release-notes template snippet (what changed/how validated) for future versions.
- TODO [P2]: Add a short doc on integrating Hydra RTL into other projects (AXI shell expectations).
- DONE: Add a short “how to file a good bug” blurb (logs to attach, FRAME_DUMP, LOG_KEYS/LOG_FRAMES).
- TODO [P2]: Add a glossary of module names/prefixes (cam_/cfg_/sel_/dbg_) for newcomers.
- TODO [P2]: Add a short doc showing example outputs from libhydra tools (info/irq_test) to set expectations.
