...
- **TODO [P2]:** Extend `scripts/check_required_files.py` or add a companion script to verify every tracker referenced in `docs/TODO_MASTER_INDEX.md` actually exists under `docs/toodo/` (not just a fixed list). Run it in CI so renames can't slip.
...
- **TODO [P3]:** Broaden `docs/toodo/todo_status_overview.md` into a tracker-agnostic “status board” that lists every domain tracker’s current state/owner so reviewers can immediately see what’s in progress vs. blocked.
...
- **TODO [P3]:** Warn when editing a tracker listed in `docs/toodo/todo_dependency_map.md` without updating its dependents (e.g., via a pre-commit hook) so the dependency graph remains accurate.
...
-- **TODO [P2]:** Provide a `docs/toodo/tree_guidelines.md` page describing directory layout conventions, when to add new top-level folders, and how to keep the toc/master index synced so contributors follow the same structuring rules.
...
- **TODO [P2]:** Build a GitHub Actions workflow that runs the full `make` + `scripts/check_required_files.py` / `scripts/todo_sweep.py` + `scripts/check_todo_unique.py` matrix and publishes `scripts/todo_inspect.py` artifacts (FPS stats, structured logs) with each run.
- **TODO [P3]:** Auto-generate GitHub issue templates tied to key trackers (FPS, rendering, DMA) so triage captures priority/context by referencing `docs/toodo/todo_system_fps.md`, `docs/toodo/todo_rendering.md`, etc.
