...
- **Verilated PCIe endpoint + cocotb/pyuvm**: build a cocotb testbench that toggles BAR0/INT_STATUS via DPI hooks. Goals: verify IRQ pulses (`msi_pulse`), DMA stub start/done, blitter stub FIFO/copy, and HDMI CRC updates. Artifact: VCD/trace for CI. (Scaffold lives in `sim/tests/cocotb_hydra`; CI runs it optionally with icarus + cocotb.)
