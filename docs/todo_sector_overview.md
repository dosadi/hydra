...
1. **RTL + AXI/IP flow (highest priority)**
   - Most active TODOs live in `docs/todo_master.md` and `docs/todo_axi_lite_coverage.md` (AXI/INT assertions, coverage, frame_done tracking).
...
2. **DRAM/SDRAM stub infrastructure**
   - `docs/todo_dram_stub.md` lists outstanding items (burst support, error handling, debugging guides, poison mode documentation).
...
4. **Simulation/viewer UX & docs**
   - `docs/todo_master.md`, `docs/todo_simulation_viewer.md`, and `docs/todo_rendering*.md` track viewer enhancements (HUD toggles, input recording, config persistence).
   - Remaining work includes joystick support, grid overlays, and HUD telemetry export.
5. **Testing/CI automation**
   - `docs/todo_testing_ci.md` currently outlines job additions (cocotb nightlies, frame diff artifacts, formatting checks).
...
6. **Multiplatform builds & docs**
   - `docs/todo_multiplatform_builds.md` and `docs/macos_windows_build.md` describe outstanding presets/notes; completed items include presets and bootstrap scripts.
...
7. **Underrepresented trackers**
   - `docs/todo_sector_overview.md` currently mirrors the big trackers but should also point to the quieter lists (site wiki, xschem, board level) so people can add new anchors when those areas need attention.
