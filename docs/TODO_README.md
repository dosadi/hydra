...
### Find all P0 items
```bash
grep -r "\[P0\]" docs/todo/todo_*.md
```

### Find all IN-PROGRESS items
```bash
grep -r "IN-PROGRESS" docs/todo/todo_*.md
```

### Count DONE items
```bash
grep -r "^- DONE:" docs/todo/todo_*.md | wc -l
```

### Search for a keyword
```bash
grep -ri "backpressure" docs/todo/todo_*.md
```

### Check a specific domain (e.g., HDMI)
```bash
cat docs/todo/todo_hdmi.md | grep "TODO"
```
