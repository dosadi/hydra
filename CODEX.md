...
## Lock Coordination (Codex-Specific)

When multiple Codex sessions work on the same repo, use the lock broker to prevent conflicts:

```bash
# Enable lock coordination
export CODEX_LOCKS=on

# The lock broker will automatically coordinate access to:
# - Build outputs (sim/obj_dir)
# - Shared files during editing
# - Session-scoped outputs
```

See `prototypes/codex-lock-broker/` for the lock broker implementation and `docs/rfc/lock_coordination.md` for design details.
...
Codex CLI sessions should coordinate through the agent integration bridge:

- Check `docs/agent_integration_bridge.md` for current work state
- Update `docs/TODO_SESSION_CONTINUATION_2025_11_25.md` with session progress
- Use lock broker for multi-session coordination
- Run `scripts/todo_sweep.py` before/after changes to update TODO counts
...
- **Short, imperative subjects** (e.g., `Clamp camera bounds`, `Add voxel edit hotkeys`)
...
- **Lock coordination**: `docs/rfc/lock_coordination.md`, `prototypes/codex-lock-broker/`</content>
<parameter name="filePath">/workspaces/hydra/CODEX.md